...
class DD:
    ...
    def transform(self):
        ...
        time_df = copy(df_copy.loc[:,["最後にいいねした時間","最後にretweetした時間","最後にtweetした時間"]])
        time_df = time_df.applymap(lambda x: int(x.timestamp())) #datetimeからtimestanpに変換
    ...
    def dead(self, days = 90):
        ...
        """
        df = self.transform()
        self.dead_ids = df.query("最新のアクションした時間 < @self.now - 60*60*24 * @days")["Username"] #30日以内に活動していないアカウントを抽出
    ...
    #フォロワー、フォロイー取得(tweepyobject)
      if follower == True:
          accounts_list = client.get_users_followers(user_id,max_results=num_of_followers).data
          
      else:
          accounts_list = client.get_users_following(user_id,max_results=num_of_accounts).data
      #print(len(accounts_list))
      #print(accounts_list)
      
      #空のDataFrame作成
      df = pd.DataFrame(columns=['Username', '最後にいいねした時間', 'いいねの回数', 'いいね間隔の標準偏差', '最後にretweetした時間', 'retweetの回数', 'retweet間隔の標準偏差', '最後にtweetした時間', 'tweetの回数', 'tweet間隔の標準偏差'])
      ...
      for user in accounts_list :
          ...
          #いいね
          likes = client.get_liked_tweets(id, max_results=num_of_tweets, tweet_fields=["created_at"]) #likeのtweetオブジェクトを取得

          if likes.data is None:
              like_count = 0
              last_like_time = datetime.fromisoformat("1970-01-01 00:00:00+00:00")
              ...
              if len(np.array(like_list[1:]) - np.array(like_list[:-1])) == 0:
                  ...
                  like_std = np.var(np.array(like_list[1:]) - np.array(like_list[:-1]))
          #ツイート、リツイート、 返信
          timeline = client.get_users_tweets(id, max_results = num_of_tweets, tweet_fields=["created_at"])
          if timeline.data is None:
              tweet_count = 0
              last_tweet_time = datetime.fromisoformat("1970-01-01 00:00:00+00:00")
              ...
              retweet_count = 0
              last_tweet_time = datetime.fromisoformat("1970-01-01 00:00:00+00:00")
              last_retweet_time = datetime.fromisoformat("1970-01-01 00:00:00+00:00")
              tweet_list = []
              retweet_list = []

              for i in range(len(timeline.data)):
                  if int(time.time()) - timeline.data[i].created_at.transform() < second_period:
                      if timeline.data[i].text[:2] == "RT":
                          retweet_count += 1
                          retweet_list.append(timeline.data[i].created_at.timestamp())
                          if timeline.data[i].created_at > last_retweet_time:
                              last_retweet_time = timeline.data[i].created_at                    
                      else:
                          tweet_count += 1
                          tweet_list.append(timeline.data[i].created_at.timestamp())
              ...
              if len(np.array(tweet_list[1:]) - np.array(tweet_list[:-1])) == 0:
                  ...
                  tweet_std = np.var(np.array(tweet_list[1:]) - np.array(tweet_list[:-1]))
                  
              retweet_list.sort()
              if len(np.array(retweet_list[1:]) - np.array(retweet_list[:-1])) == 0:
                  retweet_std = np.NaN
              else:
                  retweet_std = np.var(np.array(retweet_list[1:]) - np.array(retweet_list[:-1]))
                  
              data = [[user.username, last_like_time,like_count, like_std, last_retweet_time, retweet_count, retweet_std, last_tweet_time, tweet_count, tweet_std]]
              append_df = pd.DataFrame(data, columns = ['Username', '最後にいいねした時間', 'いいねの回数', 'いいね間隔の標準偏差', '最後にretweetした時間', 'retweetの回数', 'retweet間隔の標準偏差', '最後にtweetした時間', 'tweetの回数', 'tweet間隔の標準偏差'])
